  ...
  return (
    <div className="flat-page mx-auto max-w-5xl space-y-7 pb-24 pt-2">
      ...
      <section className="grid gap-4 md:grid-cols-[0.9fr_1.1fr]">
        ...
        <article className="rounded-3xl border border-ink-200 bg-white p-5 shadow-soft"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-ink-500">Thư viện</p><h2 className="mt-2 font-serif text-2xl font-bold text-ink-950">{books.length}<span className="text-base font-normal text-ink-400"> / {libraryLimits.total}</span></h2></div><span className="rounded-full bg-lily-50 px-3 py-1.5 text-xs font-semibold text-lily-800">{Math.max(0, libraryLimits.total - books.length)} chỗ trống</span></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-ink-100"><div className="h-full rounded-full bg-lily-600" style={{ width: `${Math.min(100, libraryLimits.total ? books.length / libraryLimits.total * 100 : 0)}%` }} /></div><div className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><span className="text-ink-500">LilyHub</span><strong className="ml-2 text-ink-900">{lilyHubSlotsUsed}/{libraryLimits.lilyhub}</strong></div><div><span className="text-ink-500">File & web</span><strong className="ml-2 text-ink-900">{externalSlotsUsed}/{libraryLimits.external}</strong></div></div></article>
      </section>
