  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-950/35" onClick={() => setIsAaPanelOpen(false)}>
    <section className="reader-panel reader-settings-sheet w-full max-w-2xl overflow-y-auto border-t border-ink-200" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="space-y-4 p-4 pb-6">
        ...
        <label className="grid grid-cols-[5.5rem_1fr] items-center gap-2"><span className="text-sm font-medium">Phông chữ</span><select value={settings.fontFamily} onChange={(e) => updateSetting('fontFamily', e.target.value as typeof settings.fontFamily)} className="h-9 min-w-0 border border-ink-200 bg-white px-2 text-sm">{['Literata', 'Merriweather', 'Playfair Display', 'Lora', 'PT Serif', 'Noto Serif', 'Be Vietnam Pro', 'Inter'].map((font) => <option key={font}>{font}</option>)}</select></label>
